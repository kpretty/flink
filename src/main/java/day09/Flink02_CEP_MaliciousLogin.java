...
public class Flink02_CEP_MaliciousLogin {
    public static void main(String[] args) throws Exception {
        ...
        }).times(2).consecutive().within(Time.seconds(5));
        ...
        SingleOutputStreamOperator<String> select = patternDS.select(new PatternSelectFunction<UserActionLogin, String>() {
            ...
            public String select(Map<String, List<UserActionLogin>> pattern) throws Exception {
                ...
                // List<UserAction> next = pattern.get("next");
                return "用户 " + start.get(0).getId() + " 在 " + start.get(0).getTs() + " ~ " + start.get(1).getTs() + " 间，恶意登录两次";
            }
        });
